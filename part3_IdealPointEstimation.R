...
num <- "114"
votes <- read.csv(paste("data/voteview/S", num, "_votes.csv", sep=""))
votes <- votes %>% select(rollnumber,icpsr,cast_code)
legis <- read.csv(paste("data/voteview/S", num, "_members.csv", sep=""))
legis <- legis[legis$chamber=="Senate",]
rcvs <- read.csv(paste("data/voteview/S", num, "_rollcalls.csv", sep=""))
